...
start "JapSpeak - Next.js Web (3000)" /D "%WEB_DIR%" cmd /c "title JapSpeak Web (3000) && color 0E && npm run dev"
ping 127.0.0.1 -n 2 >nul
...
call :KILL_PORT 8000 "FastAPI Backend"
call :KILL_PORT 3000 "Next.js Web"
powershell -NoProfile -Command "Get-CimInstance Win32_Process -Filter \"Name = 'cmd.exe'\" | Where-Object { $_.CommandLine -like '*JapSpeak*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }" >nul 2>&1
echo.
echo [XONG] Da dung cac tien trinh tren 8000/3000 va dong sach cac tab terminal.
