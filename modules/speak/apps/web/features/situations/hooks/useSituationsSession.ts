...
export function useSituationsSession(opts: UseSituationsSessionOptions = {}) {
      ...
      try {
        ...
        if (!effectiveTranscript && blob && blob.size > 1500) {
          try {
            ...
            console.debug("[useSituationsSession] Whisper fallback notice:", whisperErr);
          }
        }
      } catch (e) {
        console.debug("[useSituationsSession] Mic stop error:", e);
      }
  ...
  const handleTimeout = useCallback(async () => {
    ...
    const timeoutRes: SituationsResult = {
      ...
      score: capturedText ? 85 : 0,
      success: !!capturedText,
      isPerfect: false,
      timedOut: !capturedText,
      reactionLatencyMs: effLimit,
      userTranscript: capturedText,
      feedback: capturedText ? "Đã ghi nhận câu đối đáp của bạn!" : "Hết thời gian phản xạ! Hãy thử luyện lại tình huống này.",
      strengths: [],
      improvements: capturedText ? [] : ["Cần phản xạ câu đối đáp nhanh hơn"],
    };
