...
export function usePitchSession(opts: UsePitchSessionOptions = {}) {
  ...
  // 1. Microphone Hardware Hook with Pre-Amp Gain Booster
  ...
  const submitWithTranscript = useCallback(
    async (transcript: string) => {
      ...
      try {
        ...
        // HYBRID WHISPER AUTO-FALLBACK for quiet voices
        if (!effectiveTranscript && blob && blob.size > 1500) {
          try {
            const whisperRes = await speechApi.transcribe(blob, "base", "ja");
            if (whisperRes?.text && whisperRes.text.trim().length > 0) {
              effectiveTranscript = whisperRes.text.trim();
            }
          } catch (whisperErr) {
            console.debug("[usePitchSession] Whisper fallback notice:", whisperErr);
          }
        }
      } catch (e) {
        console.debug("[usePitchSession] Mic stop error:", e);
      }
  ...
  const handleTimeout = useCallback(async () => {
    ...
    let capturedText = latestTranscriptRef.current.trim();
    try {
      const blob = await micRef.current.stopRecording();
      if (!capturedText && blob && blob.size > 1500) {
        try {
          const whisperRes = await speechApi.transcribe(blob, "base", "ja");
          if (whisperRes?.text && whisperRes.text.trim().length > 0) {
    ...
    const timeoutRes: PitchResult = {
      ...
      score: capturedText ? 80 : 0,
      success: !!capturedText,
      isPerfect: false,
      timedOut: !capturedText,
      reactionLatencyMs: effLimit,
      userTranscript: capturedText,
      feedback: capturedText ? "Đã ghi nhận câu nói của bạn!" : "Hết thời gian phản xạ! Hãy thử luyện lại câu này.",
      strengths: [],
      improvements: capturedText ? [] : ["Cần phản xạ phát âm nhanh hơn"],
    };
