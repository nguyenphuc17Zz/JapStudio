...
export function useReflexSession(opts: UseReflexSessionOptions) {
  ...
  const [isWhisperRescuing, setIsWhisperRescuing] = useState(false);
  const [isWhisperRescued, setIsWhisperRescued] = useState(false);
  ...
  const startNext = useCallback(async () => {
    try {
      ...
      setError(null);
      setResult(null);
      setIsWhisperRescued(false);
      setIsWhisperRescuing(false);
      reactionLatencyRef.current = null;
      ...
      // Notify user if AI generation fell back to static pool
      if (ex.ai_generated === false) {
        window.dispatchEvent(
          new CustomEvent("app-toast", {
            detail: {
              type: "warning",
              message: "⚠️ AI tạm thời không khả dụng — đang dùng bài tập từ kho tĩnh.",
            },
          })
        );
      }
  ...
  const handleTimeout = useCallback(async () => {
    ...
    let capturedText = latestTranscriptRef.current || speechPreviewRef.current.interimTranscript.trim();
    let rescuedByWhisper = false;

    // HYBRID WHISPER AUTO-FALLBACK: If user spoke quietly and Web Speech timed out with empty text
    if (!capturedText && audioBlob && audioBlob.size > 1500) {
      setIsWhisperRescuing(true);
      try {
        const whisperRes = await speechApi.transcribe(audioBlob, "base", "ja");
        if (whisperRes?.text && whisperRes.text.trim().length > 0) {
          capturedText = whisperRes.text.trim();
          rescuedByWhisper = true;
          setIsWhisperRescued(true);
        }
      } catch (whisperErr) {
        console.debug("[useReflexSession] Whisper timeout fallback notice:", whisperErr);
      } finally {
        setIsWhisperRescuing(false);
      }
    }
    ...
    const payload: any = {
      ...
      timed_out: !rescuedByWhisper,
      late_response: false,
      speech_confidence: rescuedByWhisper ? 0.95 : 0,
      response_speed_ms: currentEx.timerLimitMs,
    };

    try {
      const res = await reflexApi.submitAttempt(currentEx.id, payload);
      const mapped: ReflexResult = mapApiResult(res, currentEx, true, userAudioUrl, capturedText, rescuedByWhisper);
      setResult(mapped);
  ...
  const submitWithTranscript = useCallback(
    async (transcript: string, opts?: { semanticLatencyMs?: number; late?: boolean }) => {
      ...
      // HYBRID WHISPER AUTO-FALLBACK: If Web Speech missed the quiet voice, call Whisper AI!
      if (!effectiveTranscript && audioBlob && audioBlob.size > 1500) {
        setIsWhisperRescuing(true);
        try {
          const whisperRes = await speechApi.transcribe(audioBlob, "base", "ja");
          if (whisperRes?.text && whisperRes.text.trim().length > 0) {
            effectiveTranscript = whisperRes.text.trim();
            rescuedByWhisper = true;
            setIsWhisperRescued(true);
          }
        } catch (whisperErr) {
          console.debug("[useReflexSession] Whisper submit fallback notice:", whisperErr);
        } finally {
          setIsWhisperRescuing(false);
        }
      }
      ...
      try {
        const res = await reflexApi.submitAttempt(currentEx.id, payload);
        const mapped = mapApiResult(res, currentEx, false, userAudioUrl, effectiveTranscript, rescuedByWhisper);
        setResult(mapped);
        setResults((r) => [...r, mapped]);
        updateStats(mapped);
        setPhase("result");
...
function mapApiResult(
  ...
  return {
    ...
    isWhisperRescued: !!isWhisperRescued,
  };
}
