...
export function ReflexPromptCard({
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-2xl border border-border/70 dark:border-white/10 bg-card/60 dark:bg-white/[0.02] shadow-xs transition-all duration-300 w-full", className)}>
      {/* Top Header Strip */}
      <div className="bg-muted/40 border-b border-border/60 px-3.5 sm:px-4 py-2 flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap">
        <div className="flex items-center gap-1.5 shrink-0">
          <Badge variant={modeInfo.color} size="sm" className="font-bold text-[11px] whitespace-nowrap">
            {modeInfo.ja} • {modeInfo.label}
          </Badge>
        </div>

        <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground shrink-0 whitespace-nowrap">
          {rc.jlpt_level && (
            <Badge variant="jlpt" size="sm" className="font-extrabold text-[10px] px-1.5 py-0">
              {rc.jlpt_level}
            </Badge>
          )}
          {rc.frequency_badge && (
            <Badge variant="kintsugi" size="sm" className="font-bold text-[10px] px-1.5 py-0">
              {rc.frequency_badge}
            </Badge>
          )}
          <span className="px-1.5 py-0.5 rounded-md bg-background border text-[10px] uppercase font-bold tracking-wider">
            {exercise.difficulty || "Normal"}
          </span>
          <span className="text-primary font-mono font-bold text-[11px] px-1.5 py-0.5 rounded bg-primary/10 border border-primary/20">
          ...
          {(exercise as any).ai_generated === false ? (
            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-100 text-amber-700 border border-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800/50 leading-none">
              📚 Static
            </span>
          ) : (exercise as any).ai_generated === true ? (
            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold bg-violet-100 text-violet-700 border border-violet-200 dark:bg-violet-950/40 dark:text-violet-400 dark:border-violet-800/50 leading-none">
              ✨ AI
            </span>
          ) : null}
        </div>
      ...
      <div className="p-4 sm:p-5 space-y-3">
        {isKeigoVocab ? (
          ...
          <div className="text-center space-y-4">
            ...
            <div className="flex flex-col items-center justify-center gap-2">
              ...
              {subtitleMode !== "hidden" && keigoSubjectHint && (
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-muted/80 border border-border/80 text-xs font-bold text-foreground shadow-2xs whitespace-nowrap">
            ...
            {subtitleMode === "hidden" ? (
              ...
              <div className="space-y-3">
                ...
                <div className="w-full max-w-lg mx-auto p-3 rounded-2xl bg-amber-500/10 border border-amber-500/25 shadow-xs space-y-1.5 animate-in fade-in zoom-in duration-200 text-center">
                  <div className="flex items-center justify-center gap-2 flex-wrap">
                    <ArrowRight className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400 shrink-0 animate-pulse" />
                    <span className="text-xs md:text-sm font-black text-amber-700 dark:text-amber-300 whitespace-nowrap">
                      Nói ngay: {keigoTargetLabel}
                    </span>
                  </div>

                  {keigoFormula && (
                    <div className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground pt-0.5 flex-wrap">
                      <span className="text-[10px] uppercase font-sans font-extrabold text-amber-600 dark:text-amber-400 whitespace-nowrap">
                        💡 Công thức:
                      </span>
                      <span className="font-jp text-xs font-bold text-foreground bg-card/80 px-2.5 py-0.5 rounded-lg border border-border/70 whitespace-nowrap">
                        {keigoFormula}
                      </span>
                    </div>
                  )}
          ...
          <div className="text-center space-y-4">
            ...
            <div className="flex items-center justify-center gap-2 flex-wrap mb-1">
              ...
              {((exercise as any)?.rank || rc.rank) && (
                <span className="text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 flex items-center gap-1 shadow-2xs">
                  <Sparkles className="w-3 h-3 text-amber-500" />
                  Top #{(exercise as any)?.rank || rc.rank} BCCWJ
                </span>
              )}
            ...
            {subtitleMode === "hidden" ? (
              ...
              <div className="space-y-3">
                ...
                <div className="w-full max-w-lg mx-auto p-3 rounded-2xl bg-violet-500/10 border border-violet-500/25 shadow-xs space-y-1.5 animate-in fade-in zoom-in duration-200 text-center">
                  <div className="flex items-center justify-center gap-2 flex-wrap">
                    <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground whitespace-nowrap">
                      Phản xạ tiếng Nhật:
                    </span>
                    <span className="text-xs font-extrabold text-violet-600 dark:text-violet-400 bg-violet-500/15 px-2.5 py-0.5 rounded-lg border border-violet-500/30 whitespace-nowrap">
                      Nói to từ vựng
                    </span>
                  </div>
                  {rc.collocation_ja && (
                    <div className="flex items-center justify-center gap-1.5 text-xs font-medium text-foreground pt-0.5 flex-wrap">
                      <span className="text-muted-foreground whitespace-nowrap">💡 Cụm tự nhiên:</span>
                      <span className="font-jp text-xs font-bold text-violet-600 dark:text-violet-400 bg-card/90 px-2.5 py-0.5 rounded-lg border border-border/80 whitespace-nowrap">
                        {rc.collocation_ja}
                      </span>
                      {rc.collocation_vi && (
                        <span className="text-[11px] text-muted-foreground whitespace-nowrap">({rc.collocation_vi})</span>
                      )}
                    </div>
                  )}
                </div>
          ...
          subtitleMode === "hidden" ? (
            ...
            <div className="text-center space-y-3.5">
              ...
              {targetDetail ? (
                <div className="w-full max-w-lg mx-auto p-3 rounded-2xl bg-primary/10 border border-primary/25 shadow-xs space-y-1.5 animate-in fade-in zoom-in duration-200 text-center">
                  {/* Tầng 1: Yêu cầu hành động chính (1 dòng duy nhất, chống ngắt chữ giữa chừng) */}
                  <div className="flex items-center justify-center gap-2 flex-wrap text-center">
                    <span className="text-[11px] font-extrabold uppercase tracking-wider text-muted-foreground whitespace-nowrap">
                      Chia sang:
                    </span>
                    <Badge variant="sakura" size="sm" className="font-extrabold font-jp text-xs shrink-0 whitespace-nowrap shadow-2xs">
                      {targetDetail.formJa}
                    </Badge>
                    <ArrowRight className="h-3.5 w-3.5 text-primary shrink-0 animate-pulse" />
                    <span className="text-sm md:text-base font-black text-primary tracking-tight whitespace-nowrap">
                      {targetDetail.shortName}
                    </span>
                  </div>

                  {/* Tầng 2: Ý nghĩa / Gợi ý mẫu tinh gọn */}
                  {targetDetail.meaning && (
                    <div className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground pt-0.5 flex-wrap">
                      <span className="text-[11px] font-bold text-primary/80 whitespace-nowrap">💡 Ý nghĩa:</span>
                      <span className="font-semibold text-foreground/90 bg-card/80 px-2.5 py-0.5 rounded-lg border border-border/70 whitespace-nowrap">
                        {targetDetail.meaning}
                      </span>
                    </div>
                  )}
                ...
                <div className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-2xl bg-primary/10 border border-primary/25 shadow-xs animate-in fade-in zoom-in duration-200 mx-auto whitespace-nowrap">
                  <span className="text-xs font-bold text-muted-foreground whitespace-nowrap">Chuyển sang:</span>
                  <ArrowRight className="h-3.5 w-3.5 text-primary shrink-0" />
                  <span className="text-sm md:text-base font-black font-jp text-primary whitespace-nowrap">
                    {target}
                  </span>
                </div>
              ) : null}
          ...
          <div className="text-center space-y-2 max-w-xl mx-auto">
            ...
            {isTransformation && subtitleMode !== "hidden" && (
              <div className="w-full max-w-lg mx-auto p-3 rounded-2xl bg-primary/10 border border-primary/25 shadow-xs space-y-1.5 animate-in fade-in duration-200 text-center">
                {transformTargetLabel && (
                  <div className="flex items-center justify-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs md:text-sm font-black font-jp text-primary whitespace-nowrap">
                      {transformTargetLabel}
                    </span>
                  </div>
                )}
        ...
        <div className="flex items-center justify-center gap-3 pt-1">
          <button
            ...
            onClick={onPlayAudio}
            className={cn(
              "inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all shadow-xs border cursor-pointer",
              isPlaying
          ...
          >
            ...
            <kbd className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-background/80 border border-border/80 text-muted-foreground font-bold shadow-2xs ml-1">L</kbd>
          </button>
        </div>
      </div>
    </div>
