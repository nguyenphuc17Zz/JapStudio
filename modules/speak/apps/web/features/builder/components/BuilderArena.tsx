...
import { Volume2, CheckCircle2, XCircle, Eye, EyeOff, Mic, Clock, Trophy, Sparkles, RotateCcw, ArrowRight } from "lucide-react";
import { Button } from "@/components/ui/button";
import { speakJapaneseText, stopWebSpeech } from "@/features/speaking/services/web-speech";
import { UniversalFurigana } from "@/components/japanese/UniversalFurigana";
import { soundFX } from "@/lib/sound-fx";
import { cn } from "@/lib/utils";
import type { BuilderResult, ClauseSpan, BuilderExercise } from "../services/builder-api";

const KIND_STYLE: Record<string, string> = {
  keyword: "bg-sky-500/15 text-sky-700 border-sky-500/30",
  connector: "bg-emerald-500/15 text-emerald-700 border-emerald-500/30",
  nominalizer: "bg-violet-500/15 text-violet-700 border-violet-500/30",
  ending: "bg-amber-500/15 text-amber-700 border-amber-500/30",
  other: "bg-muted text-foreground border-border",
};

export function ClauseMap({ clauses }: { clauses: ClauseSpan[] }) {
  if (!clauses || clauses.length === 0) return null;
  return (
    <div className="flex flex-wrap gap-1.5">
      {clauses.map((c, i) => (
        <span key={i} className={cn("font-jp rounded-lg border px-2 py-1 text-sm", KIND_STYLE[c.kind] || KIND_STYLE.other)}>
          {c.text}
        </span>
      ))}
    </div>
...
interface BuilderResultCardProps {
...
export function BuilderResultCard({ result, exercise, isPending = false, liveTranscript = "", onNext, autoNext, onCancelAutoNext, onRetry }: BuilderResultCardProps) {
  ...
  const canonicalDisplay =
    ...
    (exercise as any)?.extra_metadata?.builder_config?.reference ||
  ...
  const canonicalVi =
    ...
    "";
  // Model audio text: ALWAYS play model sentence, NEVER user's transcript!
  ...
  if (!result && !isPending) {
    return (
      <div className="h-full min-h-[280px] flex flex-col items-center justify-center gap-2 rounded-3xl border border-dashed border-border/80 bg-card/50 p-6 text-center">
        <p className="font-jp text-2xl text-muted-foreground/60">文立て</p>
        <p className="text-xs text-muted-foreground">Xây 1 câu dài — kết quả và clause map sẽ hiện ở đây</p>
      </div>
    );
  }
  ...
  const dims = [
    { k: "coverage", label: "Bao phủ", v: a.coverage },
    { k: "connection", label: "Nối câu", v: a.connection },
    ...
    <div className={cn("rounded-3xl border p-3 sm:p-3.5 shadow-lg transition-all animate-in fade-in zoom-in-95 duration-200 washi-texture h-full flex flex-col justify-between overflow-hidden gap-2", statusConfig.borderClass)}>
      ...
      <div className="flex-1 min-h-0 flex flex-col justify-between rounded-2xl bg-primary/[0.02] dark:bg-primary/[0.04] border border-primary/20 p-2.5 sm:p-3 relative overflow-hidden transition-all duration-300">
        <div className="flex flex-col justify-center h-full gap-1.5 text-center">
          <p className="text-[11px] font-black uppercase tracking-wider text-primary flex items-center justify-center gap-1.5"><Sparkles className="h-3.5 w-3.5 text-amber-500" /><span>Câu mẫu bản xứ:</span></p>
          ...
          {canonicalVi && (
            <p className="text-xs text-muted-foreground italic font-sans">{canonicalVi}</p>
          )}
          {result?.clauses && result.clauses.length > 0 ? <div className="mt-1"><ClauseMap clauses={result.clauses} /></div> : null}
        </div>
      </div>
...
export function BuilderSummary({ results, onRestart, onToPlan }: { results: BuilderResult[]; onRestart?: () => void; onToPlan?: () => void }) {
