...
export function useAudioRecorder(options: UseAudioRecorderOptions = {}) {
  ...
  // Global event listener for real-time gain sync across components and tabs
  useEffect(() => {
  ...
  const monitorVolume = useCallback(() => {
    ...
    const checkVolume = () => {
      ...
      // throttle volume updates to 60ms for responsive metering of quiet voices
      if (now - lastVolumeUpdateRef.current >= 60) {
        ...
        const normalized = Math.min(1.0, Math.max(0.0, avg / 80.0));
        
        if (Math.abs(normalized - currentVolumeRef.current) > 0.01 || (normalized === 0 && currentVolumeRef.current !== 0)) {
  ...
  const requestPermission = useCallback(
    async (deviceId?: string): Promise<boolean> => {
      ...
      try {
        ...
        // 2. Dynamics Compressor: boosts quiet whispers while preventing harsh clipping
        ...
        // 3. Analyser for Volume Metering
        ...
        // Pipeline: source -> gainNode -> compressor -> analyser (UI)
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      // Record from amplified Pre-Amp destination stream if available
