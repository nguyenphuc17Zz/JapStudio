...
import { ArrowLeft, Youtube, Sparkles, AlertCircle, RefreshCw, CheckCircle2 } from "lucide-react";
...
// Inline job status tracker (replaces deleted ImportProgressTracker)
function InlineJobTracker({
  jobId,
  onCompleted,
  onFailed,
}: {
  ...
  const [stage, setStage] = React.useState("Đang xử lý...");
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      while (!stopped) {
        try {
          const job = await shadowingApi.getJobStatus(jobId);
          setStage(job.stage || "Đang phân tích...");
          setStatus(job.status);

          if (job.status === "completed" || job.status === "ready") {
            onCompleted(job.video_id);
            return;
          }
          if (job.status === "failed") {
            onFailed(job.error_message || "Phân tích thất bại");
            return;
          }
        } catch (e: any) {
  ...
  return (
    <div className="p-8 sm:p-12 rounded-2xl bg-card border border-border shadow-lg space-y-5 max-w-lg mx-auto text-center animate-in fade-in">
      <div className="relative mx-auto w-16 h-16 flex items-center justify-center">
        <div className="absolute inset-0 rounded-full bg-primary/20 animate-ping opacity-60" />
        <div className="relative p-4 rounded-2xl bg-primary/15 text-primary border border-primary/25">
          <Sparkles className="h-8 w-8" />
        </div>
      </div>
      <div className="space-y-1">
        <h2 className="text-base font-bold text-foreground">AI đang phân tích video...</h2>
        <p className="text-xs text-muted-foreground">{stage}</p>
      </div>
      <div className="flex items-center justify-center gap-2 text-xs text-primary font-bold">
        <RefreshCw className="h-3.5 w-3.5 animate-spin" />
        <span>Trích xuất phụ đề & Furigana tự động</span>
      </div>
    </div>
  );
}
