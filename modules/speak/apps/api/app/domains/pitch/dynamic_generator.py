...
def compute_pitch_mora_helpers(
    ...
    """Splits Hiragana reading into mora breakdown and produces visual pitch notation (e.g., [L]-H-[H] or Downstep)."""
    moras: list[str] = []
    i = 0
...
class AIPitchGenerator:
    ...
    async def _dispatch_smart_cached_exercise(
        ...
        """Orchestrates Adaptive Bandit, Spaced Decay Zero-Latency Serving, and Async Pool Growth for Pitch Accent."""
        is_explore, pool_size = await self.cache_service.should_explore(
            domain="pitch", sub_mode=sub_mode, difficulty=difficulty
        )
        if not is_explore:
        ...
        if fresh and fresh.get("ai_generated") is not False:
            async def _save_bg(item):
                try:
                    ...
                    logger.warning(f"[AIPitchGenerator] Background pool save failed: {err}")
