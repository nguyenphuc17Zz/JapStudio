...
class AIReflexGenerator:
        ...
        if fresh and fresh.get("ai_generated") is not False:
            async def _save_bg(item):
                try:
                    ...
                    logger.warning(f"[AIReflexGenerator] Background pool save failed: {err}")
    ...
    async def generate_dynamic_exercise(
        ...
        **kwargs,
        ...
        """Routes reflex generation to specialized AI prompt templates, smart cache pool, or high-speed deterministic factory."""
        try:
            if sub_mode == "reflex_conjugation":
                ...
                return await self._dispatch_smart_cached_exercise(
                    ...
                    difficulty=difficulty,
                    pressure_level=pressure_level,
                    generator_coro_factory=lambda: self._generate_dynamic_transformation(
                        difficulty=difficulty,
                        pressure_level=pressure_level,
                        user_id=user_id,
                ...
                cat = kwargs.get("context_category")
                return await self._dispatch_smart_cached_exercise(
                    sub_mode=sub_mode,
                    difficulty=difficulty,
                    pressure_level=pressure_level,
                    generator_coro_factory=lambda: self._generate_dynamic_context(
                        difficulty=difficulty,
                        pressure_level=pressure_level,
                        user_id=user_id,
                ...
                # Speed Q&A — smart cache pool + AI-generated
                return await self._dispatch_smart_cached_exercise(
                    sub_mode=sub_mode,
                    difficulty=difficulty,
                    pressure_level=pressure_level,
                    generator_coro_factory=lambda: self._generate_dynamic_qna(
                        difficulty=difficulty,
                        pressure_level=pressure_level,
                        user_id=user_id,
                    ),
    ...
    async def _generate_dynamic_qna(
        ...
        try:
            ...
            raw_text = getattr(resp, "text", getattr(resp, "content", "")).strip()
