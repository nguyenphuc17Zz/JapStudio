...
class SpeechTopicGenerator:
    ...
    async def _dispatch_smart_cached_topic(
        ...
        """Orchestrates bandit exploration, spaced decay zero-latency serving, and async expansion for monologue topics."""
        is_explore, pool_size = await self.cache_service.should_explore(
            domain="monologue", sub_mode=sub_mode, difficulty=difficulty
        )
        if not is_explore:
            ...
            if cached:
                try:
                    spec = SpeechTaskSpec(
                        ...
                        genre=SpeechGenre(cached["genre"]),
                        topic_domain=SpeechTopicDomain(cached["topic_domain"]),
                        difficulty=int(cached["difficulty"]),
                        expected_duration_sec=int(cached.get("expected_duration_sec", 60)),
                        prep_duration_sec=int(cached.get("prep_duration_sec", 15)),
                        support_level=SpeechSupportLevel(int(cached.get("support_level", 1))),
                        support=SpeechSupport(
                            keywords=cached.get("support", {}).get("keywords", []),
                            outline=cached.get("support", {}).get("outline", []),
                            guided_questions=cached.get("support", {}).get("guided_questions", []),
                        ),
                        constraints=cached.get("constraints", []),
                        learning_targets=cached.get("learning_targets", []),
                        outline_hint=cached.get("outline_hint", []),
                    ...
                    logger.warning(f"[SpeechTopicGenerator] Failed to deserialize cached spec: {err}")

        fresh_spec = await generator_coro_factory()
        if fresh_spec:
            cached_dict = {
                "topic": fresh_spec.topic,
                "instruction": fresh_spec.instruction,
                "genre": fresh_spec.genre.value if hasattr(fresh_spec.genre, "value") else str(fresh_spec.genre),
                "topic_domain": fresh_spec.topic_domain.value if hasattr(fresh_spec.topic_domain, "value") else str(fresh_spec.topic_domain),
                "difficulty": fresh_spec.difficulty,
                "expected_duration_sec": fresh_spec.expected_duration_sec,
                "prep_duration_sec": fresh_spec.prep_duration_sec,
                "support_level": int(fresh_spec.support_level.value if hasattr(fresh_spec.support_level, "value") else fresh_spec.support_level),
                "support": {
                    "keywords": fresh_spec.support.keywords,
                    "outline": fresh_spec.support.outline,
                    "guided_questions": fresh_spec.support.guided_questions,
                },
                "constraints": fresh_spec.constraints,
                "learning_targets": fresh_spec.learning_targets,
                "outline_hint": fresh_spec.outline_hint,
                "prompt": fresh_spec.topic,
                "ai_generated": True,
            ...
            async def _save_bg(item):
                try:
                    ...
                    logger.warning(f"[SpeechTopicGenerator] Background pool save failed: {err}")

            asyncio.create_task(_save_bg(cached_dict))
        return fresh_spec
    ...
    async def generate(
        ...
        sub_mode = genre.value if hasattr(genre, "value") else str(genre)
        category = domain.value if hasattr(domain, "value") else str(domain)
        diff_str = str(difficulty)

        cached_or_fresh = await self._dispatch_smart_cached_topic(
            sub_mode=sub_mode,
            difficulty=diff_str,
            category=category,
            generator_coro_factory=lambda: self._generate_validated_topic(
                user_id=user_id,
                level=level,
    ...
    async def _generate_validated_topic(
        self,
        user_id: str,
        level: str,
        speaking_level: str,
