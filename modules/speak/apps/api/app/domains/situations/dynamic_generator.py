...
class AISituationsGenerator:
    ...
    async def generate_dynamic_exercise(
        ...
        elif not category or category not in SITUATIONAL_CATEGORIES:
        ...
        if not is_custom and fresh and fresh.get("ai_generated") is not False:
            async def _save_bg(item):
                try:
                    ...
                    logger.warning(f"[AISituationsGenerator] Background pool save failed: {err}")

            asyncio.create_task(_save_bg(fresh))
        return fresh

    async def _generate_raw_ai_situation(
        self,
        category: str | None,
