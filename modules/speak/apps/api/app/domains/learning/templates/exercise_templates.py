...
EXERCISE_TEMPLATES: dict[str, dict[str, Any]] = {
    ...
    # 25-26 Aizuchi Dojo Templates (v1) — Mode 7
    "aizuchi.reaction.v1": {
        "template_key": "aizuchi.reaction.v1",
        "exercise_type": ExerciseType.AIZUCHI_REACTION.value,
        "item_type_affinity": LearningItemType.FLUENCY.value,
        "template_version": "v1",
        "title_template": "相づちリアクション: {target_title}",
        "objective_template": "Chêm aizuchi đúng lúc, đúng loại trong cửa sổ pause của NPC.",
        "scenario_template": "NPC kể chuyện liên tục 3-5 câu, mỗi câu có khoảng lặng — bạn chêm へー/うんうん/確かに/それで？",
        "instruction_template": "Nghe NPC nói. Khi đèn xanh bật (khoảng lặng), chêm ngay 1-2 từ tự nhiên, đúng mối quan hệ casual/business.",
        "prompt_frame": "Tạo chuỗi NPC turns ngắn cho {target_title} với expected backchannel types rõ.",
        "expected_pattern_rules": ["{target_pattern}"],
        "default_estimated_minutes": 5,
    },
    "warikomi.interrupt.v1": {
        "template_key": "warikomi.interrupt.v1",
        "exercise_type": ExerciseType.WARIKOMI_INTERRUPT.value,
        "item_type_affinity": LearningItemType.NATURALNESS.value,
        "template_version": "v1",
        "title_template": "割り込み: {target_title}",
        "objective_template": "Chen ngang đúng lúc NPC lấy hơi, lịch sự, không đè giữa chữ.",
        "scenario_template": "NPC nói dài không ngừng — bạn phải chen vào lúc pause bằng câu lịch sự phù hợp register.",
        "instruction_template": "Nghe NPC, đợi khoảng lặng rồi chen: casual 「ちょっと待って！」/ business 「すみません、ちょっとよろしいでしょうか」.",
        "prompt_frame": "Tạo NPC monologue dài cho {target_title} với điểm pause rõ để chen ngang.",
        "expected_pattern_rules": ["{target_pattern}"],
        "default_estimated_minutes": 5,
    },
    # 27-29 Sentence Builder Templates (v1) — Mode 8
    "builder.assemble.v1": {
        "template_key": "builder.assemble.v1",
        "exercise_type": ExerciseType.SENTENCE_ASSEMBLE.value,
        "item_type_affinity": LearningItemType.FLUENCY.value,
        "template_version": "v1",
        "title_template": "文立て: {target_title}",
        "objective_template": "Nối từ khóa rời thành 1 câu dài tự nhiên với clause-chaining đúng.",
        "scenario_template": "4 từ khóa rời + starter gợi ý — bạn nói 1 câu dài dùng hết từ khóa.",
        "instruction_template": "Nghe/nhìn từ khóa, nói 1 câu dài tự nhiên, chú ý kỹ năng nối {target_title}.",
        "prompt_frame": "Tạo cụm từ khóa + starter cho {target_title} với 1 focus clause skill rõ.",
        "expected_pattern_rules": ["{target_pattern}"],
        "default_estimated_minutes": 4,
    },
    "builder.expand.v1": {
        "template_key": "builder.expand.v1",
        "exercise_type": ExerciseType.SENTENCE_EXPAND.value,
        "item_type_affinity": LearningItemType.NATURALNESS.value,
        "template_version": "v1",
        "title_template": "文拡大: {target_title}",
        "objective_template": "Mở rộng câu cụt thành câu dài bằng cách thêm mệnh đề.",
        "scenario_template": "1 câu cụt + yêu cầu thêm mệnh đề (lý do/kết quả/điều kiện) — bạn nói lại câu dài.",
        "instruction_template": "Nghe câu gốc và yêu cầu mở rộng, nói câu dài tự nhiên giữ ý gốc.",
        "prompt_frame": "Tạo câu cụt + expand requirement cho {target_title}.",
        "expected_pattern_rules": ["{target_pattern}"],
        "default_estimated_minutes": 4,
    },
    "builder.repair.v1": {
        "template_key": "builder.repair.v1",
        "exercise_type": ExerciseType.SENTENCE_REPAIR.value,
        "item_type_affinity": LearningItemType.NATURALNESS.value,
        "template_version": "v1",
        "title_template": "文修理: {target_title}",
        "objective_template": "Sửa câu lủng củng kiểu sách thành bản tự nhiên như bản xứ.",
        "scenario_template": "1 câu lủng củng (dài dòng/khô cứng/sai register) — bạn nói lại bản native.",
        "instruction_template": "Nghe câu lủng củng, nói lại tự nhiên: nối mệnh đề + contraction + sentence-end đúng register.",
        "prompt_frame": "Tạo câu lủng củng + fix hint cho {target_title}.",
        "expected_pattern_rules": ["{target_pattern}"],
        "default_estimated_minutes": 4,
    },
    # 30-32 Interpret Templates (v1) — Mode 9 VI-JA
    "interpret.word.v1": {
        "template_key": "interpret.word.v1",
        "exercise_type": ExerciseType.INTERPRET_WORD.value,
        "item_type_affinity": LearningItemType.VOCABULARY.value,
        "template_version": "v1",
        "title_template": "越日単語: {target_title}",
        "objective_template": "Nói ngay tiếng Nhật cho từ/cụm tiếng Việt trong 8s.",
        "scenario_template": "Từ/cụm tiếng Việt (Tết/công sở/đời sống) — bạn nói ngay tương đương Nhật.",
        "instruction_template": "Nghe/nhìn từ Việt, nói ngay tiếng Nhật tương đương, đúng register.",
        "prompt_frame": "Tạo từ/cụm Việt + expected JA keywords cho {target_title}.",
        "expected_pattern_rules": ["{target_pattern}"],
        "default_estimated_minutes": 3,
    },
    "interpret.sentence.v1": {
        "template_key": "interpret.sentence.v1",
        "exercise_type": ExerciseType.INTERPRET_SENTENCE.value,
        "item_type_affinity": LearningItemType.FLUENCY.value,
        "template_version": "v1",
        "title_template": "越日文: {target_title}",
        "objective_template": "Dịch câu Việt sang Nhật giữ đủ ý, đúng SOV, tự nhiên.",
        "scenario_template": "Câu tiếng Việt đời sống/công sở — bạn dịch sang Nhật, không dịch逐字.",
        "instruction_template": "Nghe câu Việt, nói câu Nhật giữ đủ ý chính, đúng trật tự SOV + trợ từ.",
        "prompt_frame": "Tạo câu Việt + expected JA keywords + reference cho {target_title}.",
        "expected_pattern_rules": ["{target_pattern}"],
        "default_estimated_minutes": 4,
    },
    "interpret.situation.v1": {
        "template_key": "interpret.situation.v1",
        "exercise_type": ExerciseType.INTERPRET_SITUATION.value,
        "item_type_affinity": LearningItemType.NATURALNESS.value,
        "template_version": "v1",
        "title_template": "越日通訳: {target_title}",
        "objective_template": "Phiên dịch tình huống Việt cho người Nhật, đúng keigo.",
        "scenario_template": "Tình huống công sở/đời sống (lịch Tết, xin nghỉ, giải thích) — bạn trình bày bằng Nhật.",
        "instruction_template": "Nghe tình huống, giải thích bằng tiếng Nhật tự nhiên, đúng mức lịch sự.",
        "prompt_frame": "Tạo tình huống + expected JA keywords + reference cho {target_title}.",
        "expected_pattern_rules": ["{target_pattern}"],
        "default_estimated_minutes": 5,
    },
}
